...
To provide access to the Spring `ApplicationContext` in the knowledge base, the `SpringPlugin` instance should be created, *configured as a Spring bean and added to the {sponge} engine*. Note that Spring plugin shouldn't be defined in {sponge} XML configuration file. See the following example.
...
----
<1> {sponge} configured as a Spring bean. Note that `SpringEngine` implementation is used here in order to startup and shutdown the engine by Spring. `DefaultEngine` could also be used here but it wouldn't provide automatic startup and shutdown.
<2> Added `SpringPlugin`.
<3> `SpringPlugin` configured as a Spring bean.
...
class SpringTrigger(Trigger):
    def configure(self):
        self.eventName = "springEvent"
    def run(self, event):
        beanValue = spring.context.getBean("testBean") # <1>
        self.logger.debug("Bean value = {}", beanValue)
----
<1> A Spring bean named `"testBean"` is acquired from the Spring `ApplicationContext` by using `SpringPlugin` instance referenced by a variable `spring`.
...
The *sponge* component provides integration bridge between Apache Camel and {sponge} engine. It allows:

* to route a body of a Camel message to the {sponge} engine by converting it to {sponge} event (producer endpoint),
* to route a message from an {sponge} knowledge base to a Camel route (consumer endpoint).
...
|`action`
|`"CamelProducerAction"`
|Could be used only on the producer side of the route. It will synchronously call an {sponge} action that has a name specified by the value of this option.
...
==== CamelPlugin
`CamelPlugin` provides an interface to the Camel context so it may be used in a knowledge base. `CamelPlugin` may be configured in three different ways.

* Explicitly as a Spring bean and assigned to the engine using Engine Builder API. This is the preferred way.
+
.Example
[source,python]
----
...
----
* Implicitly when creating an {sponge} Camel endpoint.
* Explicitly in {sponge} XML configuration file.
+
.Example
[source,xml]
----
...
<sponge xmlns="http://www.openksavi.org/sponge" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <plugins>
        <!-- Note: don't change this plugin name. -->
...
`send(body)`:: Sends the body to all current consumers.
`send(uri, body)`:: Sends the body to an endpoint.
`request(body)`:: Sends the body to an endpoint returning any result output body.
`getContext()`:: Returns a Camel context.
`getConsumers()`:: Returns the current list of consumers.

==== Spring-based support
`EngineCamelConfiguration` provides base Camel and {sponge} configuration using Spring Java configuration. For example your Spring configuration could inherit from this class.
...
Spring bean named `springPlugin` is an instance of `SpringPlugin` that could be registered in the engine and used in knowledge bases as a `spring` variable.

Spring bean named `camelPlugin` is an instance of `CamelPlugin` that could be registered in the engine and used in knowledge bases as a `camel` variable.
...
Camel producer action is invoked by {sponge} synchronously when a Camel exchange comes to the {sponge} engine. The result returned by this action is placed as the body of the Camel _IN_ message. So it may be used by the next endpoint in the route if there is any.
