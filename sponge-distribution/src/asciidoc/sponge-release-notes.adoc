...
* API change: The {sponge} REST API has been renamed to the {sponge} Remote API. This change resulted in a major refactoring of the server side (`sponge-remote-api-server`) and some adjustments of the client side (`sponge-remote-api-client`). The API concepts were clarified in the documentation.
...
* Added an `ActionDelegateRemoteApiOperation` builder.
