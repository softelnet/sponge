...
* API change: Added `features` to the response header in the REST API.
* API change: Added `ProvideActionArgsRequestBody.initial` in the REST API.
...
* Added the `license` and the `description` to the REST API features.
