...
Properties could also be used as {sponge} variables for session (engine) scope. In that case you have to set attribute `variable` to `true`. Note that the type of such variables is always `String`.
