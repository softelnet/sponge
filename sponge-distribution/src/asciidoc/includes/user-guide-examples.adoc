...
[discrete]
=== OpenStreetMap tiles servers usage
The example action `Action with a geo map` (`ActionWithGeoMap`) uses the OpenStreetMap tiles servers to show a base map. Please see the https://operations.osmfoundation.org/policies/tiles/[Tile Usage Policy] requirements.
