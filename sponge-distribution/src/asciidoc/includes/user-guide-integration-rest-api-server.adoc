...
[source,bash,subs="verbatim,attributes"]
----
docker run -it --rm -v `pwd`:/opt/tmp openksavi/sponge:latest ./sponge -k "classpath*:/org/openksavi/sponge/restapi/server/kb_add_remote_api_user.py" /opt/tmp/password.txt
----
