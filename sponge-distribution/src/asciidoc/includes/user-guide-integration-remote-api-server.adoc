...
== Custom operations
You can define a custom Remote API operation (using the `ActionDelegateRemoteApiOperation` class in the route builder) that delegates a Remote API request to an action call (e.g. to allow implementing an operation body in a scripting language but providing a static Remote API interface).
