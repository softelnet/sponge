...
A web gRPC client requires a https://grpc.io/docs/tutorials/basic/web/[proxy] to forward a gRPC browser request to a backend {sponge} gRPC service. In that case the convention is that the proxy port is the REST API port plus `2`.
