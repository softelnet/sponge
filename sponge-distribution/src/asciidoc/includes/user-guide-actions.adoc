...
|Sets a callable flag for the action. A callable action must have an `onCall` method defined. Defaults to `true`. The reason an action would be not callable is if it is used as an _action form_ with provided arguments.
...
class MultipleArgumentsAction(Action):
    def onConfigure(self):
        ...
        self.withArgs([
            ...
            ArgMeta("decimalListArg", ListType(ObjectType().withClassName("java.math.BigDecimal"))),
            ArgMeta("stringArrayArg", ObjectType().withClassName("java.lang.String[]")),
            ArgMeta("javaClassArg", ObjectType().withClassName("org.openksavi.sponge.examples.CustomObject")),
            ArgMeta("javaClassListArg", ListType(ObjectType().withClassName("org.openksavi.sponge.examples.CustomObject"))),
...
class SetActuator(Action):
    def onConfigure(self):
        ...
        self.withArgs([
            ArgMeta("actuator1", StringType()).withLabel("Actuator 1 state").withProvided(ProvidedMeta().withValue().withValueSet()),
            ArgMeta("actuator2", BooleanType()).withLabel("Actuator 2 state").withProvided(ProvidedMeta().withValue()),
            ArgMeta("actuator3", IntegerType().withNullable()).withLabel("Actuator 3 state").withProvided(ProvidedMeta().withValue().withReadOnly()),
            ArgMeta("actuator4", IntegerType()).withLabel("Actuator 4 state"),
            ArgMeta("actuator5", StringType()).withLabel("Actuator 5 state").withProvided(ProvidedMeta().withValue().withValueSet().withDependency("actuator1"))
        ]).withNoResult()
    ...
    def onProvideArgs(self, context):
        ...
        if "actuator5" in context.names:
            context.provided["actuator5"] = ProvidedValue().withValue(sponge.getVariable("actuator5", None)).withValueSet(["X", "Y", "Z", context.current["actuator1"]])
