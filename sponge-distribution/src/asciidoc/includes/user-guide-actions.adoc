...
|`optional`
|A not mandatory flag specifying if this argument is optional (defaults to `false`). Optional arguments may be specified only as last in the argument list. Optional arguments are not required to be passed when calling an action.
|===
...
class MultipleArgumentsAction(Action):
    def onConfigure(self):
        ...
        self.argsMeta = [
            ...
            ArgMeta("javaClassArg", ObjectType("org.openksavi.sponge.examples.CustomObject").tag("complex")),
