...
The alias for the base class for script-based filters is `Filter`. The base class for Java-based filters is `JavaFilter`.
...
The alias for the base class for script-based triggers is `Trigger`. The base class for Java-based filters is `JavaTrigger`.
...
|*`event` or `events`*
|These properties should be used to set the names of the events that cause this trigger to fire. They may be used only as setters. They are shortcuts for `setEvent(eventName)` and `setEvents(eventNames)` methods respectively. You may use only one of them. The property `event` allows setting one event name. The property `events` is an array of event names. This property is mandatory. It should be set in the `onConfigure` callback method.
...
The alias for the base class for script-based rules is `Rule`. The base class for Java-based rules is `JavaRule`.
...
|`onInit()`
|The initialization callback method that is invoked before creating every new rule instance.
...
|Sets conditions for an event specified by an alias (or event name if aliases are not used). A condition is a method of this class or a closure/lambda that is invoked to verify that a new incoming event corresponds to this rule. The name of the condition method is irrelevant.
...
Event alias:: An optional alias for the event. The alias is a unique (in the scope of the rule) name assigned to the event. Aliases are mandatory if there are more than one events of the same type (i.e. having the same name). When each of the events is of different type, there is no need to specify an alias. In such case aliases will be defined automatically and equal to the name of the corresponding event.
...
Event specification should be formatted as text `"eventName [eventAlias [:eventMode"]]`. White characters between all elements are allowed. For example specifications `"event1 e1 :first"`, `"event1"`, `"event1 e1"` define the first event named `"event1"`.

The following examples of complete event specifications assume that the rule has a duration that spans over all incoming events listed in the second column. The integer value in the brackets is the `id` of the event. An element `null` means that the event hasn't happened. Incoming events: `e1[1]`, `e2[2]`, `e2[3]`, `e3[4]`, `e2[5]`, `e3[6]`, `e3[7]`.
...
CAUTION: A condition in the form of a closure or a lambda specifies two arguments: a rule instance (determined at the runtime) and an event instance. Take care not to mix up the `rule` argument with `this` (in Java) or `self` (in Python) as they are references to different objects.
...
public class SameSourceJavaRule extends JavaRule { // <1>
    ...
    public void onConfigure() {
        ...
        setConditions("e1", "severityCondition"); // <3>
        setConditions("e2", "severityCondition", (EventCondition) (rule, event) -> { // <4>
...
The alias for the base class for script-based correlators is `Correlator`. The base class for Java-based correlators is `JavaCorrelator`.
...
|*`events`*
|This property should be used to set the names of events that this correlator listens to. It may be used only as a setter. It is a shortcut for `setEvents(eventNames)` method. This mandatory property must be set in the `onConfigure` callback method.
...
|`onInit()`
|The initialization callback method that is invoked before creating a new correlator instance. This method is optional.

|`boolean onAcceptAsFirst(Event event)`
|Checks if the event should be accepted as the first event of a correlator, therefore starting a new working instance. A method `onAcceptAsFirst` is invoked after `onInit`. This method is optional. The default implementation returns `true`.
...
class SampleCorrelator(Correlator): # <1>
    ...
    def onInit(self): # <5>
        self.eventLog = [] # <6>
    def onAcceptAsFirst(self, event): # <7>
        return event.name == "filesystemFailure" # <8>
...
<5> The correlator initialization callback method. It is invoked before `onAcceptAsFirst`.
<6> Setting an initial value to the field `eventLog`.
<7> The correlator `onAcceptAsFirst` callback method.
<8> The correlator will accept as the first an event named `filesystemFailure`.
...
<2> Setting the class attribute `eventLog`.
