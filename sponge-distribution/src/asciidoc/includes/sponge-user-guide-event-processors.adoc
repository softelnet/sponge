...
The `run` method is called when a sequence of events specified by this rule has happened and all the conditions have been fulfilled. The argument `event` is the reference to the final event that caused this rule to fire. There could be many such sequences fitting the rule definition.
...
|Returns the instance of the event that already happened and that has a specified alias. This method may be used inside `run` method. If an event hasn't happened yet, this method throws an exception. This method may return `null` only when an event that supposed not to happen didn't occur as specified.
...
<3> The correlator configuration callback method.
<4> Define that the correlator is supposed to wait for events `"filesystemFailure"` and `"diskFailure"` (in no particular order).
<5> Set the maximum number of concurrent instances allowed for this correlator. Any first event matching the names will be accepted as the first event, starting the instance of this correlator.
