...
`setEvent(eventName)` / `setEvents(eventNames)`:: These methods set names of filtered events.
...
`setEvent(eventName)` and `setEvents(eventNames)`:: These methods set names of the events that cause this trigger to fire.
...
`boolean acceptsAsFirst(Event event)`:: Checks if the event should be accepted as the first event of a correlator, therefore starting a new working instance. `acceptsAsFirst()` is invoked after `init()`. This method is optional. The default implementation returns `true`.
...
`setMaxInstances()`:: The maximum number of concurrent instances allowed for this correlator. If this value is not set, there will be no limit of concurrent instances. In that case you will probably need to implement `acceptsAsFirst()` method.
