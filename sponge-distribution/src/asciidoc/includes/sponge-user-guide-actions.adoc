...
Metadata for argument and result types support types that are represented by classes in the `org.openksavi.sponge.type` package. In order to provide more flexibility in a client code a type specification may be extended by a _format_ and _tags_ which are texts not interpreted by the {sponge} engine but could be interpreted by a client code.
...
class MultipleArgumentsAction(Action):
    def onConfigure(self):
        ...
        self.argsMeta = [
            ...
            ArgMeta("binaryArg", BinaryType().mimeType("image/png").format("28x28").tags("drawing")),
        ]
