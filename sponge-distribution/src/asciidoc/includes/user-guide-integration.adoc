...
|`String`
|The Java regular expression of a line from the process output stream. The thread that started the process will wait (blocking) for such line. May be `null` if the thread shouldn't wait for a specific line (or `waitForErrorOutputLineRegexp` if set).
