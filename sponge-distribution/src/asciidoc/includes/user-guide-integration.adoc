...
WARNING: Due to the integration issues with the external library, this plugin may not work correctly if connecting to newer versions of the MPD server. As an alternative you may use the https://www.musicpd.org/clients/mpc/[mpc] command line interface wrapped as an external process. For an example see the {sourcesUrl}/sponge-examples-projects/sponge-examples-project-mpd-mpc[MPD/MPC demo project].
...
class SendSms(Action):
    def onConfigure(self):
        ...
        self.argsMeta = [
            ArgMeta("recipient", StringType()).withLabel("Recipient").withDescription("The SMS recipient."),
            ArgMeta("message", StringType()).withLabel("Message").withDescription("The SMS message.")
        ]
