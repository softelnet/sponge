...
|`inputRedirect`
|`InputRedirect`
|The standard input redirect type (see the following tables).

|`outputRedirect`
|`OutputRedirect`
|The standard output redirect type (see the following tables).

|`errorRedirect`
|`ErrorRedirect`
|The standard error redirect type (see the following tables).
...
|`FILE` |Sets the subprocess input as the `ProcessConfiguration.inputFile` file specified as the file name.
...
|`LOGGER` |Logs the subprocess standard output to the logger (as INFO).
|===
...
|`LOGGER` |Logs the subprocess error output to the logger (as WARN).
|===

.Example of running an external executable with arguments in Java
[source,java]
----
ProcessInstance process = engine.getOperations().runProcess(
        ProcessConfiguration.builder("echo").arguments("TEST").outputRedirect(OutputRedirect.STRING).build());
...
[source,java]
----
ProcessInstance process = engine.getOperations().runProcess(ProcessConfiguration.builder("printenv")
        .arguments("TEST_VARIABLE").env("TEST_VARIABLE", "TEST").outputRedirect(OutputRedirect.STRING).build());
----
        ...
        <plugin name="py4j" class="org.openksavi.sponge.py4j.GatewayServerPy4JPlugin">
            <configuration>
                <pythonScript>
                    ...
                    <outputRedirect>LOGGER</outputRedirect>
                </pythonScript>
