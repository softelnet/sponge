...
== Features
The following screenshots show the key features of the mobile application.
...
The access to actions in the mobile application is generic. However the application includes a demo of recognizing a handwritten digit. If the current connection points to a {sponge} instance that has the required action that performs the digit recognition, this demo is enabled in the navigation drawer.
