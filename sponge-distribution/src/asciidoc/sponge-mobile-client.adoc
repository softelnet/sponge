...
|`ListType`
|Editing is supported with limitations (see the <<mobile-advanced-use-cases,Advanced use cases>> chapter). Viewing not supported. This functionality is experimental.
...
|`icon`
|Action
|An action icon name. The supported set of icons is limited to the https://cdn.materialdesignicons.com/3.4.93/[material design icons (currently v3.4.93)].
...
|`filename`
|`BinaryType`
|A file name associated with a binary value.
...
|`phone`
|A phone number format. Applicable only for `StringType`.

|`email`
|An email format. Applicable only for `StringType`.

|`url`
|An URL format. Applicable only for `StringType`.
|===
