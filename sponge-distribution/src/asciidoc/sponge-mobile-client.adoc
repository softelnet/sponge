...
Unless noted otherwise in the release notes, the {appName} and the {sponge} Flutter API library are compatible with the {sponge} service that has the same Remote API protocol version.
...
.Specific substitution notation
[cols="1,2,4"]
|===
...
|`/argumentName`
|argument source
|An action argument relative to the action arguments root, e.g. `"/header"` corresponds to an argument named `header` of the action. If the argument name is missing (e.g. `"/"`), all action arguments as a record type (represented by a map value) are taken into account.
...
.Provided arguments in an interactive form
[source,python]
----
...
|`IconInfo` or `String`
|Action
|An action icon. The icon feature converter supports an instance of `IconInfo` or `String` (an icon name) as a value. The supported set of icon names is limited to the https://cdn.materialdesignicons.com/5.6.55/[material design icons (currently v5.6.55)]. _Note that some icons may become deprecated from version to version._
...
|`url`
|`StringType`
|A `StringType` that represents an URL.
|===
