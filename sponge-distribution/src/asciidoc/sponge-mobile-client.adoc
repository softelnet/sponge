...
A source argument name could be a path if a source value is a record, e.g. `"arg1.field1"`. The `@this` keyword can be used as the source argument name. In case of a context action for an action it is a record of all parent action arguments. In case of a record argument or a list element it is the value itself.
...
==== Result substitution
A context action result can be assigned to a parent action argument. As a parent argument name you can use the `@this` keyword which means all action arguments or a whole record value or a list element.
