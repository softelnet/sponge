= Sponge Remote - The Mobile Client Application
...
=== Action call
When an action call screen is displayed, provided arguments will be fetched from the server.
...
The application may be switched to the dark or light theme in the settings.
...
|`icon`
|Type
|An type icon. Currently supported only for list elements.
...
==== Geographical map
A geographical map feature is represented by the `GeoMap` class that contains layers (instances of `GeoLayer`).
...
.Geographical position properties
[cols="1,1,3"]
|===
|Property |Type |Description
...
image::screens/mobile/digits_drawing.jpg[title="The digit recognition - drawing a digit",width=300,pdfwidth=35%,scaledwidth=35%]
