...
== Filters
Filters are used for allowing only certain events to be processed by other event processors (triggers, rules and correlators). In order to define your own filter, the class inherited from `Filter` should be created. The name of a filter is the name of the class defining this filter.
...
== Correlators
Correlators could be viewed as a generalized form of rules. In fact rules and correlators are two types of event set processors. Correlators detect correlations between events and could also be used to implement any complex event processing that isn't provided by filters, triggers or rules.
...
Every correlator should implement abstract `acceptsAsFirst` and `onEvent` methods:
...
=== Important methods
`configure()`:: A configuration callback method that is invoked when the correlator is being enabled. In this method it should be established for what type of events this correlator listens. Optionally a correlator duration could be set. This method is mandatory.
`init()`:: An initialization callback method that is invoked before creating a new correlator instance. This method is optional.
`boolean acceptsAsFirst(Event event)`:: Checks if the event should be accepted as the first event of a correlator, therefore starting a new working instance. `acceptsAsFirst()` is invoked after `init()`. This method is mandatory.
`onEvent(Event event)`:: A callback method invoked when an event that a correlator listens to happens. This method is mandatory.
...
`finish()`:: A final method that should be invoked, in `onEvent(Event event)` method, when the correlator has done its work. Only by invoking `finish()` this instance of the correlator is closed and its resources are released.
`setDuration()`:: Optionally the time how long a correlator lasts (represented as `java.time.Duration`). The instance of a correlator will be active only for a given period of time since the arrival of the first *accepted as first* event. After that time on the instance of this correlator the `onDuration()` callback method will be invoked.
`setSynchronous(synchronous)`:: The method sets a boolean _synchronous flag_ for a correlator. For details see a description of this flag for rules.
`setDisplayName(name)`:: The method sets a display name of this correlator.
...
<7> The correlator `acceptsAsFirst` callback method.
<8> The correlator will accept as first any event that it listens to but only when no other instance of `SampleJavaCorrelator` is created.
<9> The correlator `onEvent` callback method.
<10> Adds a new event to `eventLog`.
<11> Logs message.
<12> This correlator instance will finish when `4` fitting events are collected into `eventLog`.
