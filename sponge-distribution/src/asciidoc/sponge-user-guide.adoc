...
include::sponge-user-guide-architecture.adoc[leveloffset=+1]


== Main components

include::sponge-user-guide-configuration.adoc[leveloffset=+2]

include::sponge-user-guide-knowledge-bases.adoc[leveloffset=+2]
...
The name of the event is also the type of the event, so all events than have the same name are of the same type. Event names should follow Java naming conventions for variable names.
...
==== Event queues
The maximum size of a queue of events can be set in the configuration file as `eventQueueMaxSize`. One should pay attention that this parameter defines the maximum size of each queue in the system. When the maximal size is reached every next try of adding an event will cause an exception. Giving value `-1` as `eventQueueMaxSize`, you decide that the queue is unlimited (it is set by default).

The queue works according to the FIFO but the priorities of the events are also supported.
...
The priority may be assigned only to control events.
...
* The event goes directly to the Input Queue or is scheduled to be inserted to the Input Queue later. Scheduling is performed by Event Scheduler (events which are executed periodically or after a delay) or Cron (events which are to be processed at precisely given time).
...
*Creating an event means creating an instance of an event class. Sending an event means that the created event will be put into the Input Event Queue, to be processed by filters and then by triggers, rules and aggregators.*
...
** *Triggers* -  event processors that execute a specified code when an event occurs.
** *Event set processors* - event processors that correlate and process sets of events.
*** *Rules* - event set processors that correlate ordered sequences of events.
*** *Aggregators* - event set processors that are used to implement any complex event processing that isn't provided by filters, triggers or rules.
* *Actions* - processors that are used to provide functionality similar to functions. They don't listen to events.

include::sponge-user-guide-event-processors.adoc[leveloffset=+2]


=== Actions
Actions are used to provide functionality that is to be used in many knowledge bases that are written in different languages.
...
. Initialize the plugin by invoking the method `onInit()`.
. Start up the plugin by invoking the method `onStartup()`.
...
include::sponge-user-guide-integration.adoc[leveloffset=+1]
...
=== Aggregators
Aggregators should be used when filters, triggers and rules functionality is not sufficient for the problem you try to solve.
...
* `LatestNewsAggregator` aggregator listens to `news` events and stores the latest news in `storagePlugin` plugin in a Python `deque`. The number of latest news is configured as `latestNewsMaxSize` property.
* When `alarm` event occurs, this fact is logged by `AlarmTrigger` trigger using `echoPlugin` plugin and `EmphasizeAction` action.
...
* When `alarm` event occurs, not only `AlarmTrigger` (as described in the previous example) handles that event, but here also `ForwardAlarmTrigger` trigger, defined in `consumer` knowledge base. It sends the alarm message to:
...
=== Features examples
The features examples shows how to use certain functionalities. They are placed in `sponge-examples/examples` (see {sourcesUrl}/sponge-examples/examples[sources^]).
...
|`aggregators`
ifdef::useReferences[(link:sponge-examples.html#aggregators-py[py^], link:sponge-examples.html#aggregators-rb[rb^], link:sponge-examples.html#aggregators-groovy[groovy^], link:sponge-examples.html#aggregators-js[js^])]
|Shows how to use aggregators. The script defined aggregator creates an event log - a list of all events that it listens to.

|`aggregators_duration`
ifdef::useReferences[(link:sponge-examples.html#aggregators_duration-py[py^], link:sponge-examples.html#aggregators_duration-rb[rb^], link:sponge-examples.html#aggregators_duration-groovy[groovy^], link:sponge-examples.html#aggregators_duration-js[js^])]
|Shows how to use aggregators with duration. The script defined aggregator creates an event log - a list of all events that it listens to.
...
include::sponge-user-guide-standalone.adoc[leveloffset=+1]
