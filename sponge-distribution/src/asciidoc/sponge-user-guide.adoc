...
To manually enable any Java-based processors in a script knowledge base you may use `EPS.enableJava()`, `EPS.enableJavaAll()` or `EPS.enableJavaByScan()`. The default name of a Java-based processor is its full Java class name. 
...
----
def onLoad():
    EPS.enableJavaAll(SameSourceJavaRule, SameSourceJavaRule2, SameSourceJavaRule3)
----
...
To disable any Java-based processors in a script knowledge base you may use `EPS.disableJava()`, `EPS.disableJavaAll()` or `EPS.disableJavaByScan()`.
...
include::includes/sponge-user-guide-actions.adoc[leveloffset=+1]
