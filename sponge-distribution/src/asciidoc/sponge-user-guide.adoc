...
docker run --name sponge-demo -it --rm -p 8080:8080 openksavi/sponge-demo
...
docker run --name sponge-demo -it --rm -p 8080:8080 --mount type=bind,source="$(pwd)/sponge",target=/opt/sponge openksavi/sponge-demo
----
...
curl -X POST -H "Content-type:application/json" http://localhost:8080/sponge.json/v1/reload -d '{"username":"admin","password":"password"}'
