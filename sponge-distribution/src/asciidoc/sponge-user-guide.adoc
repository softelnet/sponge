...
:mpdSourcesUrl: {sourcesUrl}/sponge-external/sponge-mpd/examples/mpd/
...
|`name`
|A type location name. It is required if a type is used to specify an action argument metadata. In that case a type name is a name of an action argument. It is also required if a type is used to specify a record type field. A type location name has to set in a type constructor.
...
|`annotated`
|A flag that tells if a value of this type is annotated, i.e. wrapped by an instance of `AnnotatedValue`. Defaults to `false`. An annotated value allows dynamic passing a label, a description and features along with the value, e.g. `AnnotatedValue(imageBytes).withLabel("Image1").withFeatures({"filename":imageFilename})`, where the first property is the annotated value, the second is the label and the third is the features map that may be used in a client code.
...
|`optional`
|An optional flag specifying if a value in a location corresponding to this type is optional. Defaults to `false`. It is used in action arguments.
...
|`valueSet`
|The flag specifying if this type value set is provided. A value set is a list of allowed values. This list may be limited (the default setting) or not limited. A limited set means that the type value can be chosen only from the value set.
...
See the <<provided_arguments,provided action arguments>> as an example of using provided types.
...
==== REST API Demo Service
The {sourcesUrl}/sponge-examples-projects/sponge-examples-project-demo-service[Demo Service] use case shows how to deploy the REST API as a servlet. It uses <<rest-api-server-simple-security-strategy,the simple security strategy>>.
...
|`mpd_events`
ifdef::useReferences[({mpdSourcesUrl}/mpd_events.py[py])]
|Shows MPD events.

|`mpd_set_playlist`
ifdef::useReferences[({mpdSourcesUrl}/mpd_set_playlist.py[py])]
...
.{sponge} optional external Maven artifacts
[cols="2,2,1,5"]
|===
