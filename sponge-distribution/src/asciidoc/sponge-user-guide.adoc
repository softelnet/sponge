...
|`AnnotatedType`
|An annotated type. This type requires a `DataType` parameter, which is is a type of an annotated value. A value of this type has to be an instance of `AnnotatedValue`. For example: `AnnotatedType(BinaryType().mimeType("image/png"))`. A value of this type should be an instance of the `AnnotatedValue` class, e.g. `AnnotatedValue(imageBytes).withFeatures({"filename":imageFilename})`, where the first property is the annotated value and the second is the features map that may be used in a client code.
...
|`label`
|A processor label. This is a shortcut for `getLabel()`/`setLabel(text)` methods. A label is not used internally but could be useful in a client code.
...
|`version`
|A processor version as an integer. This is a shortcut for `getVersion()`/`setVersion(integer)` methods. A version is not used internally by the engine. However it is used to enforce version checking when calling actions via the REST API.
...
In many scripting languages properties can be accessed using dot notation rather than a direct method call. For example a label may be set using `self.label = "Label"` or `self.setLabel("Label")`.
