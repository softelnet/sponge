...
The `sponge-core` artifact uses external dependencies. If you experience conflicts in incompatible versions of Guava or Quartz, you should use the `sponge-core-shaded` artifact instead, e.g.:
...
==== Limitations
Note that Jython 2.7.1 (currently used in {sponge}) has a few bugs that could in some cases impact the stability of the system, e.g.: http://bugs.jython.org/issue2487.
...
|`sponge-tensorflow`
|Yes
|The Tensorflow integration. _Experimental, early draft, not ready to be used yet._
