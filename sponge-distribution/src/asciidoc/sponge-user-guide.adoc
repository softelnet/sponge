...
To startup the engine you should invoke the `startup()` method. After startup the engine runs in the background (i.e. using threads other than the current one). Note that the engine runs until you shutdown it.
...
|*`configure()`*
|A configuration callback method that is invoked before creating a processor. This method is mandatory.

|`init()`
|An initialization callback method that is invoked while creating a processor.
