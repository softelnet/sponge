...
|`optional`
|A flag specifying if a provided read is optional, i.e. a value may be provided implicitly, for example when other value is submitted. It is not required to be provided at all.
|===
...
If a data type is annotated and has a default value, the default value should be specified as a plain value (i.e. not wrapped in `AnnotatedValue`).
