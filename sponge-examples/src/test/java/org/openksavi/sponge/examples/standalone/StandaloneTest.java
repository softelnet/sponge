...
public class StandaloneTest {
    ...
    public void testPythonRss() {
        ...
        try {
            engineMain = TestUtils.startupStandaloneEngineMain("-c", "examples/standalone/python_rss/sponge_python_rss.xml");
            Engine engine = engineMain.getEngine();

            await().atMost(TIMEOUT, TimeUnit.SECONDS)
                    .until(() -> engine.getOperations().getVariable(Number.class, "receivedRssCount").intValue() > 1);
        } finally {
            TestUtils.shutdownStandaloneEngineMain(engineMain);
        }
    }
    ...
    public void testNews() throws InterruptedException {
        StandaloneEngineMain engineMain = null;
        try {
            engineMain = TestUtils.startupStandaloneEngineMain("-c", "examples/standalone/news/config/config.xml");
            Engine engine = engineMain.getEngine();
    ...
    public void testCamelRssNews() throws InterruptedException {
        StandaloneEngineMain engineMain = null;
        try {
            engineMain = TestUtils.startupStandaloneEngineMain("-c", "examples/standalone/camel_rss_news/config/config.xml");
            Engine engine = engineMain.getEngine();
    ...
    public void testCamelTemplate() throws InterruptedException {
        StandaloneEngineMain engineMain = null;
        try {
            engineMain = TestUtils.startupStandaloneEngineMain("-c", "examples/standalone/camel_template/config.xml");
            Engine engine = engineMain.getEngine();
