...
public final class StandaloneConstants {
    ...
    private static Options createOptions() {
        ...
        options.addOption(Option.builder(OPTION_CONFIG).longOpt(LONG_OPT_MAP.get(OPTION_CONFIG)).hasArg()
                .argName("file")
                .desc("Use the given Sponge XML configuration file. Only one configuration file may be provided.")
                .build());
        options.addOption(Option.builder(OPTION_KNOWLEDGE_BASE).longOpt(LONG_OPT_MAP.get(OPTION_KNOWLEDGE_BASE))
                .hasArg().argName("[name=]files]")
                ...
                .build());
        options.addOption(
                Option.builder(OPTION_SPRING).longOpt(LONG_OPT_MAP.get(OPTION_SPRING)).hasArg().argName("file")
        ...
        options.addOption(Option.builder(OPTION_CAMEL).longOpt(LONG_OPT_MAP.get(OPTION_CAMEL))
                .desc("Create an Apache Camel context. Works only if one or more 'spring' options are present.")
                .build());
        options.addOption(Option.builder(OPTION_INTERACTIVE).longOpt(LONG_OPT_MAP.get(OPTION_INTERACTIVE)).hasArg()
                .argName("[name]").optionalArg(true)
        ...
        options.addOption(Option.builder(OPTION_DEBUG).longOpt(LONG_OPT_MAP.get(OPTION_DEBUG))
                .desc("Enable more debugging info. "
                        + "Print all logs to the console (including exception stack traces). "
                        + "Applicable only in an interactive mode. Options '" + LONG_OPT_MAP.get(OPTION_DEBUG)
                        + "' and '" + LONG_OPT_MAP.get(OPTION_QUIET) + "' can't be used both. ")
                .build());
        options.addOption(Option.builder(OPTION_HELP).longOpt(LONG_OPT_MAP.get(OPTION_HELP))
                .desc("Print help message and exit.").build());
        ...
        options.addOption(Option.builder(OPTION_SYSTEM_PROPERTY).longOpt(LONG_OPT_MAP.get(OPTION_SYSTEM_PROPERTY))
                .hasArgs().numberOfArgs(2).valueSeparator('=').argName("property=value")
                .desc("Set the Java system property.").build());
