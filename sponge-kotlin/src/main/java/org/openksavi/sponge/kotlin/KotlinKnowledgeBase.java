 ...
 */
public abstract class KotlinKnowledgeBase extends BaseNonScriptKnowledgeBase {

    private static final Logger logger = LoggerFactory.getLogger(KotlinKnowledgeBase.class);

    protected KotlinKnowledgeBase(String name) {
        super(name, KotlinConstants.TYPE_NON_SCRIPT);
    }

    protected KotlinKnowledgeBase() {
        ...
        JvmClassMappingKt.getKotlinClass(getClass()).getNestedClasses().stream().filter(kclass -> !kclass.isAbstract())
                .forEachOrdered(kclass -> {
                    if (KotlinConstants.PROCESSOR_CLASSES.values().stream()
                            ...
                            .findFirst().isPresent()) {
                        autoEnabled.add(KotlinUtils.getProcessorName(kclass));
